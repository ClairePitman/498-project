...
function initLifespanFlower() {
    ...
    var a = {};
    unfiltered_data["1950"].nodes.forEach(function(c, b) {
        0 != b && (a[c["class"]] = !0)
    });
    var b = "";
...
function drawChart() {
    ...
    c = d3.select("#graphHolder").append("svg:svg").attr("id", "graph").attr("width",
    ...
    var e = [],
        ...
        k = {},
        d = 0;
    unfiltered_data["1950"].nodes.forEach(function(l, a) {
        if (0 == a || null != $("#" + l["class"]).attr("checked")) e.push(l), k[a] = d, d++
    });
    unfiltered_data["1950"].links.forEach(function(a, b) {
    ...
    links = c.append("g").attr("class", "linkContainer").selectAll(".link").data(g).enter().append("line").attr("class",

        "link");
    nodes = c.append("g").attr("class", "nodeContainer").selectAll(".node").data(e).enter().append("image").call(force.drag).attr("xlink:href", function(a) {
        return getImageName(a.name)
    }).attr("id", function(a) {
        return a.name
    }).attr("class", "image").attr("x", -25).attr("y", -25).attr("width", 50).attr("height", 50).attr("cursor", "pointer");
    $("svg image").tipsy({
        gravity: "w",
        html: !0,
        title: function() {
            var a = this.__data__;
            return "<span class='floatingp'>" +

                a.name + "</span><br/>Average age: <b>" + a.age + " years old</b><br/>"
        }
    });
    c.select("#root").style("cx", a).style("cy", b).style("fill", "black").attr("r", 1).on("mousedown.drag", null).on("mouseover", null);
    force.on("tick", function() {
        links.attr("x1", function(a) {
            return a.source.x
        }).attr("y1", function(a) {
            return a.source.y
        }).attr("x2", function(a) {
        ...
        nodes.attr("transform", function(a) {
...
function drawReference(a, b, c) {
    ...
    for (var e = d3.scale.linear().domain([0, ABS_MAX]).range([d3.rgb(204,229,255), d3.rgb(153,204,255)]), g = d3.scale.linear().domain([0, ABS_MAX]).range([d3.rgb(200, 150, 150), "darkred"]), k = Math.round(0.1 * maxAge), d = ABS_MAX; 0 < d; d -= k) {
        var f = ratio * d,
            l = g(d),
            m = e(d);
        a.append("circle").attr("class", "reference").attr("cx", b).attr("cy", c).attr("r", f).style("fill", m).style("stroke", "gray").style("stroke-width", "1px");
        a.append("svg:text").text(d).attr("x",

            b + f - 5 + 1).attr("y", c + 1).style("fill", "black");
